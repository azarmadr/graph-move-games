...
Defines the Canvas-based DAG visualization rendering system that displays the global directed acyclic graph of all board states, using dagre for hierarchical layout and SVG/Canvas for rendering.
...
#### Scenario: Edge color assignment
...
### Requirement: Edge arrow markers

The system SHALL render directed edge arrows: marker-end "url(#graph-arrow-move)" for Move edges, "url(#graph-arrow-spawn)" for Spawn edges.

#### Scenario: Arrow marker rendering
- **WHEN** SVG paths for edges are generated
- **THEN** Move edges use <marker> id "graph-arrow-move" (fill "#4cc9f0"), Spawn edges use <marker> id "graph-arrow-spawn" (fill "#f72585")
