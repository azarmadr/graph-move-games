...
export class GameAppElement extends HTMLElement {
  ...
  private async openGraphVisualization() {
    ...
    this.render();
    terminal.log(
      `render() done (${(performance.now() - t0).toFixed(1)}ms) — graph-tab in DOM`,
    );
  ...
  private bindEvents() {
    ...
    const resumeSelect =
      this.querySelector<HTMLSelectElement>("[data-resume-game]");
    if (resumeSelect) {
      ...
      uploadInput.addEventListener("change", () => {
        ...
        reader.onload = async () => {
          ...
          try {
            ...
            terminal.log(
              `File read done (${(performance.now() - t0).toFixed(1)}ms, ${text.length} chars)`,
            );
            terminal.log("calling importGraph...");
            const t1 = performance.now();
            const result = await importGraph(text);
            terminal.log(
              `importGraph done (${(performance.now() - t1).toFixed(1)}ms) — success=${result.success}, games=${result.games.length}`,
            );
            if (result.success && result.games.length > 0) {
              await this.refreshResumeGames();
